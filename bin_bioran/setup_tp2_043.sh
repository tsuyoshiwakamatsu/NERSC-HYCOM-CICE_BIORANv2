...
if [ "$iscycle" = "true" ]; then
     if [ ! -d data ]; then
	 echo "$WORK_HYCOM/$CONFIGNAME/expt_$NEWEXPERIMENT/data does not exist, EXIT"
     fi
elif [ "$iscycle" = "false" ]; then
     [ -d data ] && rm -rf data
     mkdir -p data/cice
fi
