...
IFS=',' read -r -a mems <<< "$members" # create an array of members
...
for imem in "${mems[@]}"; do
   ...
   if [ -f $list ]; then
      echo "rewrite fabm.yaml based on $(basename $list)"
      while IFS=": " read -r param new_value; do
         sed -i "/$param: /s/\($param: \s*\).*/\1$new_value/" "fabm.yaml"
      done < "$list"
   else
   ...
   cp -L $P/hycom_fabm.nml hycom_fabm.nml
done
...
for imem in "${mems[@]}"; do
   ...
   #cp -L $D/${fhead_rst}_mem${fmem}.a ${fhead_rst}.a
   #cp -L $D/${fhead_rst}_mem${fmem}.b ${fhead_rst}.b
done
...
for imem in "${mems[@]}"; do
   ...
   #create_symlink ${fhead_ice}.nc $D/cice/${fhead_ice}_mem${fmem}.nc # this will cause error in array job
   cp -L $D/cice/${fhead_ice}_mem${fmem}.nc ${fhead_ice}.nc
done
